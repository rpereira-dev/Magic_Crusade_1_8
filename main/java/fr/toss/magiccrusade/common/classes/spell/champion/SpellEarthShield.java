...
public class SpellEarthShield implements ISpell
{
	...
	public void animate(IMagicEntity caster, Entity target)
	{
		...
		for (int phi = -180; phi < 180; phi++)
		{
			...
			{
				if (caster.getWorld().rand.nextInt() % 100 == 0)
				{
					...
					caster.getWorld().spawnParticle(EnumParticleTypes.CRIT_MAGIC, caster.getEntity().posX,  caster.getEntity().posY,  caster.getEntity().posZ, v_x / 2.0f, v_y * 2.0f, v_z / 2.0f, 1);
				}
			}
		}
	}
	...
	{		
		caster.getEntity().addPotionEffect(new PotionEffect(Potion.absorption.id, 40, (int) (3 + stat.get_endurance() * 0.01f)));
	}
